...
export async function adminListSubCategories(
  token,
  { page = 1, limit = 50, status, search, category, mode, role, addedById, listed } = {}
) {
...
export async function adminApproveSubCategory(token, id) {
  try {
    const { data } = await api.post(
      `${subCategoriesBase()}/${encodeURIComponent(id)}/approve`,
      {},
      { headers: authHeader(token) }
    );
    return data.subCategory;
...
export async function adminRejectSubCategory(token, id) {
  try {
    const { data } = await api.post(
      `${subCategoriesBase()}/${encodeURIComponent(id)}/reject`,
      {},
      { headers: authHeader(token) }
    );
    return data.subCategory;
