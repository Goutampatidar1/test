...
import { adminGetPromotionById, adminUpdatePromotion } from "../../api/promotionController.js";
...
function validatePromotionForm(form, dateOptions = {}) {
...
export function PromotionEdit() {
  ...
  const startDateMin = startDateMinForEdit(initialStartDate);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setError("");
      try {
        const p = await adminGetPromotionById(adminToken, promotionId);
        if (cancelled) return;
        if (!p) {
          setNotFound(true);
          return;
        ...
        setInitialStartDate(p.startDate ? String(p.startDate).slice(0, 10) : "");
  ...
  const onSubmit = async (e) => {
    ...
    setSaving(true);
    try {
      await adminUpdatePromotion(adminToken, promotionId, form, imageFile);
      await Swal.fire({ icon: "success", title: "Promotion updated", timer: 1500 });
      navigate(`/admin/promo/${promotionId}`);
    } catch (e2) {
      if (e2?.status === 401) return dispatch(logout());
      await Swal.fire({ icon: "error", title: "Update failed", text: e2.message || "Could not update promotion." });
    } finally {
      setSaving(false);
    }
  };
  ...
  if (error) {
    return (
      <div className="user-page">
        <p className="user-list-error" role="alert">{error}</p>
      </div>
    );
  }
  if (loading) {
    ...
    <div className="user-page">
      <div className="user-page__toolbar">
        ...
        <div className="user-page__toolbar-text">
          <h2 className="user-page__title">Edit promotion</h2>
        </div>
        <Link to={`/admin/promo/${promotionId}`} className="btn btn--ghost">View details</Link>
      </div>
      ...
      <div className="page-card">
        <form onSubmit={onSubmit}>
          <div className="row g-3">
            ...
            <label className="user-field col-12 col-md-6">
              <span className="user-field__label">Promo banner image (optional)</span>
              <input
                ref={fileInputRef}
                type="file"
                accept="image/*"
          ...
          <div className="user-form__actions">
            <button type="submit" className="btn btn--primary" disabled={saving}>{saving ? "Saving…" : "Update Promotion"}</button>
          </div>
        </form>
      </div>
    </div>
